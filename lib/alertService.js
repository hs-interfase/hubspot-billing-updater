...
const HEARTBEAT_URL         = process.env.BETTERSTACK_HEARTBEAT_URL          // cron de días hábiles (L-V)
const HEARTBEAT_URL_WEEKEND = process.env.BETTERSTACK_HEARTBEAT_URL_WEEKEND  // cron de fin de semana (sábado)
 ...
 *
 * Hay DOS heartbeats independientes porque los crons tienen cadencias distintas:
 *   - 'weekday' (default): cronDealsBatch, corre L-V. Env BETTERSTACK_HEARTBEAT_URL.
 *   - 'weekend': cronWeekendFull, corre el sábado. Env BETTERSTACK_HEARTBEAT_URL_WEEKEND.
 * Un solo heartbeat no distingue "no corrió el weekday" de "es sábado y toca el weekend",
 * y da falsos positivos. Por eso cada cron pinguea el suyo.
 *
 * Si el heartbeat de weekend no está configurado NO se hace fallback al de weekday:
 * mejor omitir el ping (y que el monitor de weekend alerte) que pinguear el monitor
 * equivocado y esconder un miss.
 *
 * @param {'weekday'|'weekend'} kind
 */
export async function pingHeartbeat(kind = 'weekday') {
  const url = kind === 'weekend' ? HEARTBEAT_URL_WEEKEND : HEARTBEAT_URL

  if (!url) {
    const envName = kind === 'weekend' ? 'BETTERSTACK_HEARTBEAT_URL_WEEKEND' : 'BETTERSTACK_HEARTBEAT_URL'

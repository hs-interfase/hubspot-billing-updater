...
async function main() {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    try {
      ...
      // `total` = VALOR en USD (prop valor_total, principal).
      const { total, lineItemCount } = await recalcValorTotal({ dealId, applyUpdate: !DRY });
      ...
      if (DRY) {
        ...
        if (difiere) {
          changed++;
          cambios.push({ dealId, prev: Number.isFinite(prev) ? prev : null, total, lineItemCount });
          console.log(`[${i + 1}/${ids.length}] ${dealId}  ${Number.isFinite(prev) ? fmt(prev) : '(vacío)'} → ${fmt(total)}  (${lineItemCount} LIs)`);
        }
      } else {
        ...
        console.log(`[${i + 1}/${ids.length}] ${dealId}  → ${fmt(total)}  (${lineItemCount} LIs)`);
        cambios.push({ dealId, total, lineItemCount });
        changed++;
      }
    } catch (err) {
      errores++;

...
const VERIFY_ONLY = process.argv.includes('--verify-only'); // read-only: solo verifica, no sella
const DRY_RUN     = VERIFY_ONLY || !process.argv.includes('--execute');
...
async function sealAutoInvoicesForDeal(dealId, dp, stats) {
  ...
  for (const inv of invoices) {
    ...
    try {
      ...
      // changedProps con fecha_de_emision → el pipeline propaga fecha_real_de_facturacion
      // (= la fecha del período migrada) y numero_de_factura al ticket. Doble vía: además
      // propagateInvoiceStateToTicket la re-lee de la factura.
      const changed = { id_factura_nodum: AUTO_NODUM_SENTINEL };
      if (periodo) changed.fecha_de_emision = periodo;
      await runInvoiceNodumPipeline(String(inv.id), changed);
...
// ─── Verificación automática AUTO ───────────────────────────────────────────────
// Re-lee invoices + tickets del deal y confirma, por cada invoice AUTO Emitida, que:
//   factura: id_factura_nodum=11 · fecha_de_emision = fecha del período migrada
//   ticket:  numero_de_factura=11 · fecha_real_de_facturacion = misma fecha
// El período esperado = fecha_resolucion_esperada del ticket (la fuente que usó el seal).
// Read-only. Corre solo tras --execute (auto) o en modo --verify-only.
async function verifyAutoInvoicesForDeal(dealId, stats) {
  await sleep(500); // margen por consistencia eventual tras los PATCH
  let invoiceIds;
  try {
    invoiceIds = await getAllAssocIds('deals', dealId, 'invoices');
  } catch (err) {
    addFinding('VERIFY_FETCH_ERROR', 'error', { dealId, error: err.message });
    stats.errors++; return;
  }
  if (!invoiceIds.length) return;
  ...
  try {
    ...
    const tIds = await getAllAssocIds('deals', dealId, 'tickets');
    tickets = await batchReadObjects('tickets', tIds,
      ['hs_pipeline', 'hs_pipeline_stage', 'fecha_resolucion_esperada', 'of_fecha_de_facturacion',
       'numero_de_factura', 'fecha_real_de_facturacion']);
  } catch (err) {
    addFinding('VERIFY_READ_ERROR', 'error', { dealId, error: err.message });
    stats.errors++; return;
  }
  const tById = new Map(tickets.map(t => [String(t.id), t]));
  ...
  for (const inv of invoices) {
    const ip = inv.properties || {};
    if (String(ip.etapa_de_la_factura || '').trim() !== 'Emitida') continue; // solo las emitidas
    const tk = tById.get(String(ip.ticket_id || ''));
    if (!tk || String(tk.properties?.hs_pipeline || '') !== AUTO_PIPELINE) continue; // solo AUTO
    const tp = tk.properties || {};
    const periodo = toYmd(tp.fecha_resolucion_esperada) || toYmd(tp.of_fecha_de_facturacion);

    const checks = {
      nodum_inv:     String(ip.id_factura_nodum || '').trim() === AUTO_NODUM_SENTINEL,
      emision_inv:   !!periodo && toYmd(ip.fecha_de_emision) === periodo,
      nodum_tkt:     String(tp.numero_de_factura || '').trim() === AUTO_NODUM_SENTINEL,
      fechareal_tkt: !!periodo && toYmd(tp.fecha_real_de_facturacion) === periodo,
    };
    const fail = Object.entries(checks).filter(([, v]) => !v).map(([k]) => k);
    if (fail.length === 0) { ok++; continue; }
    bad++;
    console.log(`        [VERIFY ✗] inv ${inv.id} tkt ${tk.id} periodo=${periodo || '∅'} | falla: ${fail.join(',')} ` +
      `| emisionInv=${toYmd(ip.fecha_de_emision)} nodumInv=${ip.id_factura_nodum || '∅'} ` +
      `fechaRealTkt=${toYmd(tp.fecha_real_de_facturacion)} nodumTkt=${tp.numero_de_factura || '∅'}`);
    addFinding('VERIFY_MISMATCH', 'warn', { dealId, invoiceId: inv.id, ticketId: tk.id, periodo, fail });
  }
  stats.verifyOk += ok;
  stats.verifyBad += bad;
  console.log(`        [VERIFY] deal ${dealId}: ${ok} OK / ${bad} con diferencias`);
}
...
async function main() {
  ...
  // El pipeline de propagación solo se necesita al escribir (no en dry ni verify-only).
  // Import dinámico tras dotenv/config (token correcto). Correr desde la raíz del repo.
  ...
  for (const deal of deals) {
    ...
    // Modo --verify-only: solo re-lee y verifica las AUTO emitidas, sin sellar.
    if (VERIFY_ONLY) {
      stats.dealsProcessed++;
      await verifyAutoInvoicesForDeal(dealId, stats);
      continue;
    }
    ...
    for (const li of lineItems) {
      ...
      // 2026-06-28: el seal es SOLO-AUTO. NO toca tickets MANUAL — su emisión la hacen
      // Paso C/D, y el rescate de manuales atascados en forecast lo hace el script aparte
      // promoverManualForecast.mjs (forecast → "Próximos a facturar"). Por eso las ramas
      // manuales de sealPreCutoff / el else del loop quedan inalcanzables (guard de acá).
      const ticketsToSeal = tickets.filter(t => {
        const tp = t.properties || {};
        const stage = String(tp.hs_pipeline_stage || '');
        if (!isAutoPipeline(t)) { stats.manualSkipped++; return false; } // ← seal no toca manuales
    ...
    // Verificación automática tras sellar (solo en ejecución real).
    if (!DRY_RUN) await verifyAutoInvoicesForDeal(dealId, stats);
    await sleep(200);
  }
  ...
  console.log(`  AUTO emitidas:              ${DRY_RUN ? stats.autoSealedDry + (VERIFY_ONLY ? ' (verify-only)' : ' (dry)') : stats.autoSealed}`);
  console.log(`  AUTO ya Emitida (skip):     ${stats.autoAlready}`);
  console.log(`  AUTO skip (manual/sin tkt): ${stats.autoSkipped}`);
  console.log(`  MANUAL no tocados (seal):   ${stats.manualSkipped}`);
  console.log(`  VERIFY OK / diferencias:    ${stats.verifyOk} / ${stats.verifyBad}`);
